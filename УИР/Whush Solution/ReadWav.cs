...
namespace Whush.Demo
{
  ...
  class ReadWav
  {
    public ReadWav()
    { }
    public byte[] StartReadWav(string WavFileName)
    {
      ...
      fileStream.Seek(headerSize * 2, SeekOrigin.Begin);// умножать на 2????????
      ...
      int Subchun2SizeToInt = (int)header.Subchunk2Size;
      var data = new byte[Subchun2SizeToInt];
      fileStream.Read(data, 0, Subchun2SizeToInt);
